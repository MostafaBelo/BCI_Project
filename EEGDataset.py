...
class EEGDataset(Dataset):
    def __init__(self, data_dir, shard_size=50, pad_upto=10000, selective_indexing=None):
        ...
        self.current_loaded_shard = None
        ...
        if not(os.path.exists(os.path.join(data_dir, "labels.txt"))):
            raise FileNotFoundError("Labels file not found in the specified directory.")
        with open(os.path.join(data_dir, "labels.txt"), 'r') as f:
            self.labels = [line.split(":",1) for line in f.read().strip().split("\n")]
    ...
    def __getitem__(self, idx: int):
        ...
        if shard_idx != self.current_shard_idx:
            shard_path = os.path.join(self.data_dir, f"localized_eeg_sentences_ZAB_shard{shard_idx}.npy")
            self.current_loaded_shard = np.load(shard_path, allow_pickle=True)
            self.current_shard_idx = shard_idx

        data = self.current_loaded_shard[idx % self.shard_size]
        if data is None:
            return None, lbl
        ...
        if data.shape[1] < self.padding:
            ...
            if pad_width > 0:
                data = np.concatenate([data, np.zeros((data.shape[0], pad_width))], axis=1)
            else:
    ...
    def split_train_valid_test(self, train_ratio=0.8, valid_ratio=0.1, shuffle=False):
        ...
        train_dataset = EEGDataset(self.data_dir, self.shard_size, selective_indexing=train_indices)
        valid_dataset = EEGDataset(self.data_dir, self.shard_size, selective_indexing=valid_indices)
        test_dataset = EEGDataset(self.data_dir, self.shard_size, selective_indexing=test_indices)
