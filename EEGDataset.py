...
class WordEEGDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        data = current_loaded_shard[lbl_i]
        data = {**data}
        ...
        for i in range(len(data["eeg"])):
            if data["eeg"][i] is None:
                continue
            eeg_shape = data["eeg"][i].shape
            if eeg_shape[1] < self.padding:
                pad_width = self.padding - \
                    eeg_shape[1]
                if pad_width > 0:
                    data["eeg"][i] = np.concatenate(
                        [data["eeg"][i], np.zeros((eeg_shape[0], pad_width))], axis=1)
        ...
        txt = data["sentence"]
        return signal, sentiment_lbl, labels, txt
    ...
    def _collate_fn(self, batch):
        ...
        sent = torch.tensor(sent)
        labels = [item[2] for item in batch if item[0] is not None]
        txts = [item[3] for item in batch if item[0] is not None]
        return data, sent, labels, txts
    ...
    def split_train_valid_test(self, train_ratio=0.8, valid_ratio=0.1, shuffle=False):
        ...
        train_dataset = WordEEGDataset(
            self.data_dir, self.ch_count, self.shard_size, self.padding, selective_indexing=train_indices)
        valid_dataset = WordEEGDataset(
            self.data_dir, self.ch_count, self.shard_size, self.padding, selective_indexing=valid_indices)
        test_dataset = WordEEGDataset(
            self.data_dir, self.ch_count, self.shard_size, self.padding, selective_indexing=test_indices)
